...
public class TripPinServiceTest {
  ...
  public void testSelectOption() throws Exception {
    ...
    JsonNode node = getJSONNode(response);
    assertEquals(baseURL+"/$metadata#People(FirstName,LastName)/$entity", node.get("@odata.context").asText());
  ...
  public void testEntityId() throws Exception {
    ...
    JsonNode node = getJSONNode(response);
    assertEquals(baseURL+"/$metadata#People(FirstName)/$entity", node.get("@odata.context").asText());
